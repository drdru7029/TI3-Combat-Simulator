    ...
    while not valid:
        try:
            if attribute.keys()==['capacitor']: #capacitor value has a different limit than other ship attributes
                new_attribute = int(input("Enter your {} value up to {}: ".format(attribute.keys(), Ship.total_points_per_capacitor)))
                valid = True
                ...
                while new_attribute not in my_weapon_class_dict:
                    try:
                        new_attribute = input("You may enter up to {} weapons. Enter your first {} (you have {} cap remaining), "
                ...
                while weapon_count < ship.weapon_bays:
                    ...
                    while additional_weapon_validity not in my_weapon_class_dict:
                        try:
                            new_weapon_attribute_keys.append(input("Enter your next weapon, or enter 'none' to decline."))
                ...
                new_attribute = int(input("Enter your {} value up to {}: ".format(attribute.keys(), Ship.total_points_per)))
                current_totals_pre += int(new_attribute)
                if 1 <= new_attribute <= Ship.total_points_per and ship.current_total_points_all+new_attribute <= Ship.total_points_all:
                    valid = True
                    ship.current_total_points_all+=new_attribute
...
def set_ship_design_auto(ship):
    ...
    def ai_choose_weapon():
        ...
        weapon_choice_decision = random.choice(weapon_choice)
        print('weapon dec',weapon_choice_decision)
...
def combat(ship1,ship2):
    ...
    if len(player_2_state)==0:
        ...
        print('player_2_state',player_2_state)
    valid = False
    while valid==False:
        ...
        while ship1.defense > 0 and ship2.defense > 0:
            ...
            if attacking_ship==0 or attacking_ship==1 and player_2_state[0]==1:
                if ship_list[attacking_ship].capacitor > 0: #Ships cannot fire if cap has been reduced to 0
                    ...
                    while not valid2:
                        try:
                            current_weapon = (input('{}, equip your weapon, choices are: {}.'.format(ship_list[attacking_ship].ship_name,ship_list[attacking_ship].weapon_types)))
            ...
            if attacking_ship==1:
                ...
                attacking_ship_object.capacitor -= attack_strike
                Ship.capacitor_recharge = random.randrange(2, 4)
                attacking_ship_object.capacitor += Ship.capacitor_recharge
            if attacking_ship == 0:
                attacking_ship = 1
...
def manage_ships(ship1,ship2):
    ...
    while noexit:
        ...
        if option == 1:
            ...
            reset_ships(ship1)
            reset_ships(ship2)
            name = input(str(Back.CYAN + "What is the name of player 1? "))
            ...
            if p2_manual_auto==1:
                name = input(str(Back.CYAN + "What is the name of player 2? "))
