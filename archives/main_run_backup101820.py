...
master_component_dict = {'weapons': my_weapon_class_dict, 'shields':scomp.my_component_class_dict['shields'],
                         'capacitor':scomp.my_component_class_dict['capacitor']}
...
def input_controls(attribute, ship):
    ...
    while not valid:
        try:
            ...
            if list(attribute.keys())==['capacitor']:

                ship.capacitor = add_component('capacitor')


            elif list(attribute.keys())[0] in component_list: #special case for weapons, in order access weapon subclasses,
                #and apply any additional weapons--UPDATE, will be expanded to cover to all ship design components
                ...
                new_attribute = int(input("Enter your {} value up to {}: ".format(list(attribute.keys()), sc.Ship.
                                                                                  total_points_per)))
                current_totals_pre += int(new_attribute)
                if 1 <= new_attribute <= sc.Ship.total_points_per and ship.current_total_points_all+new_attribute <= \
                        sc.Ship.total_points_all:
                    valid = True
                    ship.current_total_points_all+=new_attribute
                else:
                    print(Fore.RED + "Value entered is not valid - please enter a value between 1 and {}: ".format
                    (sc.Ship.total_points_per))

        except ValueError:
            Fore.RED + "Value entered is not valid - please enter a value between 1 and {}: ".format(sc.Ship.
                                                                                                     total_points_per)
...
def set_ship_design(ship):
    ...
    # for component in ship.__dict__():
    #     print(component)

    for component in ship.constructable_components_dict:
        input_controls(component,ship)
